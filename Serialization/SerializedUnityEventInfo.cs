...
namespace ParadoxNotion.Serialization
{
    ...
    public class SerializedUnityEventInfo : ISerializedReflectedInfo
    {
        ...
        void ISerializationCallbackReceiver.OnAfterDeserialize() {
            ...
            _memberInfo = null;
            if ( result is FieldInfo && typeof(UnityEventBase).IsAssignableFrom(( result as FieldInfo ).FieldType) ) {
                _memberInfo = result;
                return;
            }
            if ( result is PropertyInfo && typeof(UnityEventBase).IsAssignableFrom(( result as PropertyInfo ).PropertyType) ) {
                _memberInfo = result;
                return;
            }
        }
